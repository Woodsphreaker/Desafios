...
const reducerSUM = (arr) => arr.reduce((_a, _b) => _a + _b);
const operators = {
    sum: (number, prop) => number + prop,
    sub: (number, prop) => number - prop,
    div: (number, prop) => number / prop,
    mul: (number, prop) => number * prop
};
const solve = (number) => {
    const algarism_1 = reducerSUM(splitNumber(number))
    const sub = operators['sub']([number], algarism_1);
    const algarism_2 = reducerSUM(splitNumber(sub));
